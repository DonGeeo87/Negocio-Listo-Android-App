...
## 🧭 Despliegue paso a paso (sin script)

1. Abre PowerShell en la raíz del proyecto:
   ```powershell
   cd C:/Users/DonGeeo87/AndroidStudioProjects/NegocioListo2
   ```
2. Copia todo el contenido actualizado de `landing/` a `public/landing/`:
   ```powershell
   Copy-Item landing\* public\landing -Recurse -Force
   ```
3. Verifica que la CLI de Firebase esté disponible:
   ```powershell
   firebase --version
   ```
   Si no está instalada, ejecuta `npm install -g firebase-tools`.
4. Despliega el hosting:
   ```powershell
   firebase deploy --only hosting
   ```
5. Al finalizar, abre la URL que muestra Firebase (por ejemplo `https://app-negocio-listo.web.app`) y confirma que los enlaces y la landing reflejan los cambios.
