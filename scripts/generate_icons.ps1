# Script para generar iconos de launcher en todas las densidades
# Usando el nuevo icono: icon-NegocioListo.png
...
$sourceIcon = "app\src\main\res\drawable\icon-NegocioListo.png"
$basePath = "app\src\main\res"

# Verificar que el archivo fuente existe
if (-not (Test-Path $sourceIcon)) {
    Write-Host "❌ Error: No se encontró el archivo fuente: $sourceIcon" -ForegroundColor Red
    exit 1
}
...
# Generar icono redondo (mismo tamaño que ic_launcher)
foreach ($density in $densities.GetEnumerator()) {
    $size = $density.Value
...
Write-Host "📱 Los iconos están listos para usar en la app" -ForegroundColor Cyan
